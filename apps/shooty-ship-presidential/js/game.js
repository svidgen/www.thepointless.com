...
SS.Bullet = function() {
	...
	this.conflicts = [SS.Enemy, SS.President, SS.Shrapnel];
...
SS.President = function() {
	...
	var image_count = 6;
	var img_id = Math.floor(1 + Math.random() * image_count);
	...
	setType(this, 'SS.President');
}; // President
SS.President.templateMarkup = SS.Enemy.templateMarkup;
Bind(SS.President, 'ss:president');
...
SS.GameOverSplash = function() {
	...
	if (score == 0) {
		...
		this.share.object = {
			text: "I scored " + score + " in Shooty Ship Presidential Edition!"
				+ " What better way to honor the president?",
			category: "game"
		};
	}
...
SS.BannerAd.templateMarkup = '\
	<!-- Shooty Ship Presidential --> \
	<ins class="adsbygoogle" \
		style="display:block" \
		data-ad-client="ca-pub-6115341109827821" \
		data-ad-slot="1023812298" \
		data-ad-format="auto" \
