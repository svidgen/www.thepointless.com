...
- Decision/change: Added pre-Route53 DNS preservation records to `deployment-config.ts` under `dnsRecordsByBranch.main`: Google Workspace MX records, the SPF TXT record, and the legacy `*.thepointless.com` CNAME to the existing CloudFront distribution. Left apex/www hosting records to CDK-managed domain/redirect deployment instead of duplicating them as arbitrary DNS records.
- Build result: `MAIN_BRANCH_DOMAIN=preview-5786.thepointless.com npm run build` succeeds; build still emits the accepted legacy `src/lib/event.cjs` warning.

## Decision — remove legacy Amplify domain association before CDK deploy (2026-07-12)
- Observed: Previous GitHub Actions deploy logs were not publicly readable without signing in, but AWS Amplify in account `054297926286` had app `dwjzqupzmdemr` (`thepointlesscom`, repo `https://github.com/svidgen/www.thepointless.com`) associated to `thepointless.com` with `www` and `dev` subdomains pointing at legacy CloudFront distribution `d37e7q7aj43e6v.cloudfront.net`.
- Decision/change: Recorded the app/domain-association details locally in gitignored file `.aws-amplify-thepointless-domain-association-removed-20260712.json` via `.git/info/exclude`, then deleted the Amplify domain association for `thepointless.com` from app `dwjzqupzmdemr` in `us-east-2` using the default AWS profile. The app itself was not deleted.
- Check result: `aws amplify list-domain-associations --profile default --region us-east-2 --app-id dwjzqupzmdemr` now returns an empty `domainAssociations` list. Next action: push/deploy CDK again and, if sanity checks pass, fully tear down the old Amplify app later.
