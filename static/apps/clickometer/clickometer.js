...
function tick() {
	if (time > 0 && !event_finished) {
		...
		document.getElementById('timer').style.marginTop = '';
		document.getElementById('timer').innerHTML = time;
		time -= 1;
...
function breakClickometer() {
	...
	var legacy_container = document.getElementById('container');
	if (legacy_container) {
		legacy_container.style.backgroundColor = 'yellow';
	}
...
globalThis.clickometer_pump = pump;
